...
INFO_MESSAGE_TEXT
  ...
  def mk_new_name_table
    ...
    # 二つ名分岐
    if nick_n <= 1
      nick_table = MK_NICK_PR_TABLE.roll(bcdice)
      d1 = nick_table.gsub(/[^\d]/, "").to_i
      nick_table.slice!(0, 16)
    elsif name_n <= 2
      nick_table = MK_NICK_FO_TABLE.roll(bcdice)
      d1 = nick_table.gsub(/[^\d]/, "").to_i
      nick_table.slice!(0, 16)
    elsif name_n <= 3
      nick_table = MK_NICK_OU_TABLE.roll(bcdice)
      d1 = nick_table.gsub(/[^\d]/, "").to_i
      nick_table.slice!(0, 14)
    elsif name_n <= 4
      nick_table = MK_NICK_TI_TABLE.roll(bcdice)
      d1 = nick_table.gsub(/[^\d]/, "").to_i
      nick_table.slice!(0, 14)
    elsif name_n <= 5
      nick_table = MK_NICK_PH_TABLE.roll(bcdice)
      d1 = nick_table.gsub(/[^\d]/, "").to_i
      nick_table.slice!(0, 15)
    else
      nick_table = MK_NICK_CO_TABLE.roll(bcdice)
      d1 = nick_table.gsub(/[^\d]/, "").to_i
      nick_table.slice!(0, 17)
    end

    # 名前分岐
    if name_n <= 1
      name_table = MK_NAME_AR_TABLE.roll(bcdice)
      d2 = name_table.gsub(/[^\d]/, "").to_i
      name_table.slice!(0, 13)
    elsif name_n <= 2
      name_table = MK_NAME_FO_TABLE.roll(bcdice)
      d2 = name_table.gsub(/[^\d]/, "").to_i
      name_table.slice!(0, 14)
    elsif name_n <= 3
      name_table = MK_NAME_DN_TABLE.roll(bcdice)
      d2 = name_table.gsub(/[^\d]/, "").to_i
      name_table.slice!(0, 14)
    elsif name_n <= 4
      name_table = MK_NAME_PL_TABLE.roll(bcdice)
      d2 = name_table.gsub(/[^\d]/, "").to_i
      name_table.slice!(0, 13)
    elsif name_n <= 5
      name_table = MK_NAME_MA_TABLE.roll(bcdice)
      d2 = name_table.gsub(/[^\d]/, "").to_i
      name_table.slice!(0, 13)
    else
      name_table = MK_NAME_GO_TABLE.roll(bcdice)
      d2 = name_table.gsub(/[^\d]/, "").to_i
      name_table.slice!(0, 13)
    end
  ...
  def mk_rare_weapon_item_random_table(num)
    ...
    case num
    when 1, 2, 3
      output = MK_NORMAL_RARE_WEAPON_ITEM_TABLE.roll(bcdice)
    else
      output = MK_ADVANCED_RARE_WEAPON_ITEM_TABLE.roll(bcdice)
    end
    output.slice!(0, 18)
    return output
  end
  ...
  def mk_rare_usual_item_random_table(num)
    ...
    case num
    when 1, 2, 3
      output = MK_NORMAL_RARE_ITEM_TABLE.roll(bcdice)
    else
      output = MK_ADVANCED_RARE_ITEM_TABLE.roll(bcdice)
    end
    output.slice!(0, 18)
    return output
  end

  MK_NORMAL_RARE_ITEM_TABLE = D66GridTable.new(
    '基本レア一般アイテム表',
    [
      %w(愚者の冠 香水 煙玉 悪名 藁人形 王妃の鏡).freeze,
      %w(星籠 転ばぬ先の杖 悟りの書 鉛の兵隊 黄金の林檎 百年茸).freeze,
      %w(愚者の冠 香水 煙玉 悪名 藁人形 王妃の鏡).freeze,
      %w(星籠 転ばぬ先の杖 悟りの書 鉛の兵隊 黄金の林檎 百年茸).freeze,
      %w(操りロープ 盗賊の七つ道具 露眼鏡 災厄王の遺物 魔法の鞍 琵琶).freeze,
      %w(兎の足 視肉 衛星帯 魔法の絨毯 軍配 聖杯).freeze,
    ].freeze
  )

  MK_NORMAL_RARE_WEAPON_ITEM_TABLE = D66GridTable.new(
    '基本レア武具アイテム表',
    [
      %w(蛍矢 小麦粉 喇叭銃 まわし しゃべる剣 大盾).freeze,
      %w(王笏 ぬいぐるみ 魔杖 獣の毛皮 バカには見えない鎧 ビキニアーマー).freeze,
      %w(蛍矢 小麦粉 喇叭銃 まわし しゃべる剣 大盾).freeze,
      %w(王笏 ぬいぐるみ 魔杖 獣の毛皮 バカには見えない鎧 ビキニアーマー).freeze,
      %w(チェインソード 輝く者 貪る者 滅ぼす者 機械の体 刈り取る者).freeze,
      %w(断ち切る者 竜の鱗鎧 射貫く者 貫く者 剥ぎ取る者 王剣).freeze,
    ].freeze
  )

  MK_ADVANCED_RARE_ITEM_TABLE = D66GridTable.new(
    '上級レア一般アイテム表',
    [
      %w(砂時計週報 兵糧丸 遊星葉書 百科辞典 夢枕 蓄音機).freeze,
      %w(砂時計週報 兵糧丸 遊星葉書 百科辞典 夢枕 蓄音機).freeze,
      %w(水晶球 狭間の棺桶 不思議なたまご 魔法瓶 不死鳥の羽飾り 紅葫蘆).freeze,
      %w(水晶球 狭間の棺桶 不思議なたまご 魔法瓶 不死鳥の羽飾り 紅葫蘆).freeze,
      %w(打ち出の小槌 消火器 滅びの予言書 召魔鏡 鉄仮面 愛).freeze,
      %w(打ち出の小槌 消火器 滅びの予言書 召魔鏡 鉄仮面 愛).freeze,
    ].freeze
  )

  MK_ADVANCED_RARE_WEAPON_ITEM_TABLE = D66GridTable.new(
    '上級レア武具アイテム表',
    [
      %w(虚弾 小鬼の襟巻 眼弾 釣竿 虹柱 服従の鞭).freeze,
      %w(虚弾 小鬼の襟巻 眼弾 釣竿 虹柱 服従の鞭).freeze,
      %w(星の杖 聖印 迷い傘 邪眼 徒手空拳 隠れ兜).freeze,
      %w(星の杖 聖印 迷い傘 邪眼 徒手空拳 隠れ兜).freeze,
      %w(太刀鋏 破城槌 黄金の鶴嘴 ムラサマ 君主の衣 蒸気甲冑).freeze,
      %w(太刀鋏 破城槌 黄金の鶴嘴 ムラサマ 君主の衣 蒸気甲冑).freeze,
    ].freeze
  )
  ...
  def mk_kingdom_environment_table(num)
    ...
    case num
    when 1
      output = MK_TECHNIC_DECIDE_TABLE.roll(bcdice)
    when 2
      output = MK_NATIONAL_STYLE_DECIDE_TABLE.roll(bcdice)
    when 3
      output = MK_RESOURCE_DECIDE_TABLE.roll(bcdice)
    when 4
      output = MK_FACILITY_DECIDE_TABLE.roll(bcdice)
    when 5
      output = MK_HUMAN_RESOURCES_DECIDE_TABLE.roll(bcdice)
    else
      output = MK_BLOOD_DECIDE_TABLE.roll(bcdice)
    end
    output.slice!(0, 11)
    return output
  end

  MK_TECHNIC_DECIDE_TABLE = Table.new(
    '技術決定表',
    '1D6',
    [
      "あなたの国は、魔法の研究、開発に力をそそぐ魔道国家である。その国のキャラクターは、星術、召喚、科学スキルの判定を行うとき、その達成値が1点上昇する（最大3点まで上昇する。4回目以降は振り直すこと）。",
      "あなたの国は、神話的遺物の逸話が残っている。レア一般アイテムの中からランダムに1種を選ぶ。そのレアアイテムのレシピを持っている。【王宮】のある部屋に、そのレア一般アイテムの名前を記入すること。",
      "あなたの国は、英雄が用いた武具の伝説が残っている。レア武具アイテムの中からランダムに1種を選ぶ。そのレアアイテムのレシピを持っている。【王宮】のある部屋に、そのレア武具アイテムの名前を記入すること。",
      "あなたの国は、有名な職人たちが揃う工業国家である。コモンアイテムを作成するとき、それらのアイテムを作成するための必要国力が1点高いものとして扱う。",
      "あなたの国は、質実剛健な兵士たちが揃っている。その国のキャラクターは、《配下》最大値が1人上昇する（最大2人まで上昇する。3回目以降は振り直すこと）。",
      "あなたの国は、過去に列強に臣従し、いまでも友好的な関係を築いている。（1D6）を振ること。1ならダイナマイト帝国、2なら千年王朝、3ならメトロ汗国、4ならハグルマ資本主義人民共和国との関係が「友好」になる。5や6なら振り直すこと。また、その列強の列強系施設1軒を獲得する。",
    ].freeze
  )

  MK_NATIONAL_STYLE_DECIDE_TABLE = Table.new(
    '国風決定表',
    '1D6',
    [
      "あなたの国は、古くからあり、伝統を重んじる気風を持つ。宮廷系施設を建設・発展するための価格が1MG軽減される（最大2MGまで軽減される。3回目以降は振り直すこと）。",
      "あなたの国は、広い国土と高い天井に恵まれている。居住系施設を建設するための価格が1MG軽減される（最大2MGまで軽減される。3回目以降は振り直すこと）。",
      "あなたの国は、夏星が豊富で、作物がたくさん収穫できる。生産系施設を建設・発展するための価格が1MG軽減される。（最大2MGまで軽減される。3回目以降は振り直すこと）。",
      "あなたの国は、しっかりとした規律と礼節があり、それを守る風潮がある。公共系施設を建設・発展するための価格が1MG軽減される（最大2MGまで軽減される。3回目以降は振り直すこと）。",
      "あなたの国は、芸術を奨励し、文化的な国民性を誇る。娯楽系施設を建設・発展するための価格が1MG軽減される（最大2MGまで軽減される。3回目以降は振り直すこと）。",
      "あなたの国は、物を大切にし、質素な生活を心がける気風を持つ。保管系施設を建設・発展するための価格が1MG軽減される（最大2MGまで軽減される。3回目以降は振り直すこと）。",
    ].freeze
  )

  MK_RESOURCE_DECIDE_TABLE = Table.new(
    '資源決定表',
    '1D6',
    [
      "あなたの国は、過去に善政がしかれ、非常に安定している。セッション開始時の《民の声》の値が1点上昇する（最大3点まで上昇する。4回目以降は振り直すこと）。",
      "あなたの国は、天然の要害に囲まれており、外敵に襲われにくい。《民》が（2D6）人増加する。",
      "あなたの国には、名工がつくった武器がある。ランダムに選んだ武具アイテム1個を獲得する。その武具アイテムはレベル1として扱う。",
      "あなたの国には、先頃友誼を誓い合った同盟国がある。王国シートの周辺階域から、ランダムに未知の土地1つを選ぶ。その土地に、王国を1つ設定すること。この国は【特産物】を持つ。「相場表」を使って、【特産物】の素材をランダムに決定すること。この国との関係は「同盟」となる。",
      "あなたの国で先頃、前王の隠し財産が発見された。《予算》を（1D6） MG獲得する。",
      "あなたの国には、隠し扉があった。「自国の地理」を決定したあと、追加で通路を2本引くことができる。通路でつながっている部屋は領土として扱う。",
    ].freeze
  )

  MK_FACILITY_DECIDE_TABLE = Table.new(
    '施設決定表',
    '1D6',
    [
      "あなたの国は、その地方を代々統治する伝統ある王国だ。宮廷系施設の中からランダムに1種を選ぶ。自国にその施設を1件建設する。",
      "あなたの国は、交易路の周囲にあり、多くの人々が流入する。居住系施設の中からランダムに1種を選ぶ。自国にその施設を1件建設する。",
      "あなたの国は、職人気質のものが多く、物作りがさかんだ。生産系施設の中からランダムに1種を選ぶ。自国にその施設を1件建設する。",
      "あなたの国は民を第一に考え、福祉に力を入れている。公共系施設の中からランダムに1種を選ぶ。自国にその施設を1件建設する。",
      "あなたの国は、歓楽国家として知られ、他国からの客もよく出入りしている。娯楽系施設の中からランダムに1種を選ぶ。自国にその施設を1件建設する。",
      "あなたの国は、辺境に位置する王国だ。周辺には怪物も少ない。保管系施設の中からランダムに1種を選ぶ。自国にその施設を1件建設する。",
    ].freeze
  )

  MK_HUMAN_RESOURCES_DECIDE_TABLE = Table.new(
    '人材決定表',
    '1D6',
    [
      "あなたの国には、高い見識を持つ知識人がいる。「才覚系生まれ表」でランダムにジョブ1種を選ぶ。そのジョブの逸材を1人獲得する。逸材の名前を決定すること。",
      "あなたの国には、皆を魅了する好人物がいる。「魅了系生まれ表」でランダムにジョブ1種を選ぶ。そのジョブの逸材を1人獲得する。逸材の名前を決定すること。",
      "あなたの国には、巧みな技術を持つ専門家がいる。「探索系生まれ表」でランダムにジョブ1種を選ぶ。そのジョブの逸材を1人獲得する。逸材の名前を決定すること。",
      "あなたの国には、見事な腕前の戦士がいる。「武勇系生まれ表」でランダムにジョブ1種を選ぶ。そのジョブの逸材を1人獲得する。逸材の名前を決定すること。",
      "あなたの国は、怪物と共存している？ （1D6）を振ること。1なら【小鬼】、2なら【ウマトカゲ】、3なら【ドワーフ】、4なら【エルフ】、5なら【キンギョ】、6なら【ごんぎつね】の《モンスターの民》を（1D6）人獲得する。",
      "あなたの国は、ここしばらく怪物や敵国の襲撃もなく、平和な日々が続いていた。《民》が（2D6）人増加する。",
    ].freeze
  )

  MK_BLOOD_DECIDE_TABLE = Table.new(
    '血族決定表',
    '1D6',
    [
      "あなたの国は、鬼族の蹂躙を受けた歴史を持ち、混血が進んでいる。その国のキャラクターは新たにスキルを修得するとき、鬼族カテゴリの自分のレベル以下のモンスターが修得しているモンスタースキルの中から、修得するスキルを選ぶことができるようになる。",
      "あなたの国は、古代に迷宮から姿を消した妖精女王の末裔といわれている。その国のキャラクターは新たにスキルを修得するとき、妖精カテゴリの自分のレベル以下のモンスターが修得しているモンスタースキルの中から、修得するスキルを選ぶことができるようになる。",
      "あなたの国は、偉大なる古龍が迷宮と化した場所であり、その尊い血を引いているといわれる。その国のキャラクターは新たにスキルを修得するとき、魔獣カテゴリの自分のレベル以下のモンスターが修得しているモンスタースキルの中から、修得するスキルを選ぶことができるようになる。",
      "あなたの国は、魔階からやってきた魔王の子供たちといわれている。その国のキャラクターは新たにスキルを修得するとき、異形カテゴリの自分のレベル以下のモンスターが修得しているモンスタースキルの中から、修得するスキルを選ぶことができるようになる。",
      "あなたの国は、死霊術師によって死者の王国に変えられた悲劇的な過去を持つ。その国のキャラクターは新たにスキルを修得するとき、死霊カテゴリの自分のレベル以下のモンスターが修得しているモンスタースキルの中から、修得するスキルを選ぶことができるようになる。",
      "あなたの国は、古代の錬金術師たちによって造られた人造生命が多数使役されている。その国のキャラクターは新たにスキルを修得するとき、呪物カテゴリの自分のレベル以下のモンスターが修得しているモンスタースキルの中から、修得するスキルを選ぶことができるようになる。",
    ].freeze
  )
  ...
  TABLES = {
    ...
    'RMS' => D66GridTable.new(
      'ランダムマップ選択表',
      [
        %w(A-1 A-1 A-2 A-2 A-3 A-3).freeze,
        %w(A-1 A-1 A-2 A-2 A-3 A-3).freeze,
        %w(B-1 B-1 B-2 B-2 B-3 B-3).freeze,
        %w(B-1 B-1 B-2 B-2 B-3 B-3).freeze,
        %w(C-1 C-1 C-2 C-2 C-3 C-3).freeze,
        %w(C-1 C-1 C-2 C-2 C-3 C-3).freeze,
      ].freeze
    ),
    'EBT' => Table.new(
    ...
    'NRUT' => MK_NORMAL_RARE_ITEM_TABLE,
    'NRWT' => MK_NORMAL_RARE_WEAPON_ITEM_TABLE,
    'ARUT' => MK_ADVANCED_RARE_ITEM_TABLE,
    'ARWT' => MK_ADVANCED_RARE_WEAPON_ITEM_TABLE,
    'NMAR' => MK_NAME_AR_TABLE,
    'NMFO' => MK_NAME_FO_TABLE,
    'NMDN' => MK_NAME_DN_TABLE,
    'NMPL' => MK_NAME_PL_TABLE,
    'NMMA' => MK_NAME_MA_TABLE,
    'NMGO' => MK_NAME_GO_TABLE,
    'TET' => MK_TECHNIC_DECIDE_TABLE,
    'NST' => MK_NATIONAL_STYLE_DECIDE_TABLE,
    'RET' => MK_RESOURCE_DECIDE_TABLE,
    'FAT' => MK_FACILITY_DECIDE_TABLE,
    'HRT' => MK_HUMAN_RESOURCES_DECIDE_TABLE,
    'BLT' => MK_BLOOD_DECIDE_TABLE,
  }.freeze
end
