...
INFO_MESSAGE_TEXT
  ...
  def mk_talent_break_table
    get_table_by_2d6([
      "寝付けないので、民と噂話に花を咲かせる。すると、経費削減のアイデアが……。[才覚/9]の判定を行う。成功すると、そのセッションの《維持費》を（1D6）MG減少できる。",
      ...
      "地図を前にして、今後の冒険について口角泡を飛ばす。意見の対立はあったが、あなたの意見が通った。我々に必要なのは英雄的死亡ではなく、卑劣な生存なのだ。 宮廷の好きなキャラクター1体を選ぶ。そのキャラクターの自分に対する《敵意》を 好きなだけ上昇させ、上昇した値だけ《民の声》を回復する。",
  ...
  def mk_search_break_table
    get_table_by_2d6([
      ...
      "寝床を探していたら、アルコーブがあり、その奥に宝箱をみつける。[探索/9]の判定を行う。成功すると、好きな素材1種類を選び、それを1D6個獲得する。",
  ...
  def mk_talent_happening_table
    get_table_by_2d6([
      "自分に王国を導くことなど可能なのだろうか……。【お酒】を1個消費することができなければ、そのセッションの間、[才覚]-1。",
      ...
      "今の自分に自信が持てなくなる。生まれ表からランダムにジョブを1つを選び、現在のジョブを、そのジョブに変更する。",
    ])
  end
  ...
  def mk_charm_happening_table
    get_table_by_2d6([
      "民同士のいさかいに心を痛め、頭髪にダメージが！ 【お酒】を1個消費することができなければ、そのセッションの間、[魅力]-1。",
      ...
      "宮廷に嫉妬の嵐が巻き起こる。宮廷の中で、あなたに対して《好意》を持つキャラクターの数を数える。そのセッションの間、行為判定を行うとき、サイコロの目の合計がこの数以下だった場合、絶対失敗となる(2未満にはならない)。",
  ...
  def mk_search_happening_table
    get_table_by_2d6([
      ...
      "流れ星に直撃。《HP》-（1D6）。",
      "敵の過去を知り、相手に同情してしまう。あなたは、このマップの支配者に対する《好意》+1。そのセッションの間、《好意》を持ったキャラクターに対して攻撃を行い、絶対失敗した場合、その《好意》の値だけ《気力》が減少する。",
      ...
      "あなたを憎む迷宮支配者が、あなたの首に賞金をかけた。このセッションの間、モンスターの攻撃やトラップの目標をランダムに決める場合、その目標は必ずあなたになる(この効果を2人以上受けた場合、この効果を受けた者の中でランダムに決定する)。",
    ])
  end
  ...
  def mk_valor_happening_table
    get_table_by_2d6([
      ...
      "自分の失敗が許せない。そのセッションの間、《器》が1点減少したものとして扱う。",
    ])
  end
  ...
  def mk_kingdom_disaster_table
    get_table_by_2d6([
      ...
      "王国が何者かに呪われる。そのセッションの間、国力を使った行為判定で選んだ2D6の目が3以下だと、絶対失敗になる。",
  ...
  def mk_kingdom_change_table
    get_table_by_2d6([
      ...
      "冒険の成功を祝う民たちが出迎えてくれる。《民の声》＋２。この結果を出したプレイヤー以外の全員は、今回の冒険を振り返り当PLのPCが《好意》を得るとしたら誰が一番ふさわしいかを協議する。決定したキャラへのPCの《好意》+1",
      ...
      "王国の子どもたちが宮廷をあなた方を見て成長する。《民》が［王国に残した《民》の数÷10＋治安レベル］D6人増える。",
  ...
  def mk_combat_fumble_table
    get_table_by_2d6([
      "敵に援軍が現れる！ 敵軍の中でもっとももっともレベルの低いモンスターが（1D6）体増える。モンスターがこの結果になった場合、好きなPCの《配下》が（1D6）体上昇する。",
  ...
  TABLES = {
    'RT' => Table.new(
      ...
      '2D6',
      [
        "神託が下る。苦難がPCを襲うが、それは救いのための試練である。そのセッションの間、PCが10点以上のダメージをモンスターから受けるたび《民の声》+1。",
        "長老が迷宮の昔話をしてくれた。この表を使用したPCが判定で失敗したとき、その判定のサイコロを振り直すことができる。この効果は、そのセッションの間に1回だけ使用できる。",
        "民は怪物の脅威に怯えている。この表を使用したPCがモンスターの《HP》を0点にすると、《民の声》+2。この効果は、そのセッションの間に1回だけ使用できる。",
        "日用品が不足しているという不満を持つ民がいるようだ。そのセッションの間、自国に「革」を5個輸送するたび《民の声》+1。",
        "民たちは王国の守りが薄いのではという不安を抱えていた。そのセッションの間、自国に「鉄」を5個輸送するたび《民の声》+1。",
        "主婦たちが食糧不足に対する不安を訴えてきた。そのセッションの間、自国に「肉」を5個輸送するたび《民の声》+1。",
        "民たちは新しい施設の建設を望んでいる。そのセッションの間、自国に「木」を5個輸送するたび《民の声》+1。",
        "武器の備えが乏しいのではないかという不安があるようだ。そのセッションの間、自国に「牙」を5個輸送するたび《民の声》+1。",
        "配下にした若者が熱心に未来を語る。この表を使用したPCは《配下》を1人消費して、《特殊配下》を1人増やす。その《特殊配下》に名前をつけ、「生まれ表」でなりたいジョブを決定すること。なりたいジョブに対応した能力値(その《特殊配下》がなりたいジョブの能力値ボーナス欄に書いてある能力値)を使った判定で、そのセッションの間に自分が絶対成功すると、その《特殊配下》は、そのジョブの逸材になる。",
        "王国は活気に満ちている。この表を使用したPCは《気力》+1、もう一度王国フェイズに行動することができる。",
        "民たちはワクワクするような冒険譚を求めている！ そのセッションのシナリオの目的を達成していたら、終了フェイズの円卓会議の開始時に、（1D6）MGが手に入る。",
      ]
    ),
    'ENC' => Table.new(
      '特殊遭遇表',
      '1D6',
      [
        "宙を舞う【グレムリン】が、宮廷の方を物欲しそうにながめている。宮廷の中で、素材欄に「機械」が含まれているアイテムを持っているPC全員は、[才覚/7+装備している素材欄に「機械」が含まれるアイテムの数]の判定を行う。失敗したPCは、そのアイテムをすべて破壊し、[装備している素材欄に「機械」が含まれるアイテムの数]D6点のダメージを受ける。",
        ...
        "災厄教の巡礼者の一団に出会う。彼らは、迷宮災厄こそおごり高ぶった人類への罰であり、悔い改めよと、その教えを説いた。《配下》を1人以上連れているキャラクターは、[魅力/自分の《配下》の数+5]の判定を行う。失敗したPC1人につき、《民の声》-1。",
      ]
    ),
    'IG' => Table.new(
      '情報収集表',
