...
INFO_MESSAGE_TEXT
  ...
  def dice_command(string, nick_e)
    ...
    when /((^|\s)(S)?SR(\d+)(([+]|[-])\d+)?($|\s))/i
      ...
      secretMarker = $3
      command = $1.upcase
      output_msg = check_seigou(command, nick_e)
  ...
  def checkRoll(string, nick_e)
    ...
    if(target > 12 )
      output  += "【#{string}】 ＞ 難易度が12を超えたため、超過分、ファンブル率が上昇！\n"
      while(target > 12)
        target = target - 1
        fumble = fumble + 1
      end
    ...
    if(fumble >= 6 )
      output += "#{getJudgeInfo(target, fumble, critical)} ＞ ファンブル率が6を超えたため自動失敗！"
      return output
    end
    
    if(target < 5 )
      output  += "【#{string}】 ＞ あらゆる難易度は5未満にはならないため、難易度は5になる！\n"
      target = 5
    end
    ...
    output += "#{getJudgeInfo(target, fumble, critical)} ＞ #{dice_str} ＞ 成功度#{total_suc}"
  ...
  def check_seigou(string, nick_e)
    debug("check_seigou begin string", string)
    
    unless(/((^|\s)S?(SR(\d+)(([+]|[-])(\d+))?($|\s)))/i =~ string)
      return '1'
    end
    ...
    output = "#{nick_e}:〔性業値〕#{target}、「修正値」#{modify} ＞ ダイス結果：（#{dice}） ＞ #{dice}＋（#{modify}）＝#{diceTotal} ＞ #{seigou}"
    
    if( dice == 12 )
      output += " ＞ 6ゾロのため〔性業値〕が1点減少！"
    end
    if( dice == 2 )
      output += " ＞ 1ゾロのため〔性業値〕が1点上昇！"
    end
    
    debug( 'check_seigou result output', output )
    return output
  end
  ...
  def rollDiceCommand(command)
    ...
    when "NPCT"
      ...
      #2d6
      result = get2d6TableResult(command, counts, operator, value)
    end
  ...
  def get2d6TableResult(command, counts, operator, value)
    result = []
    ...
    counts.times do |i|
      ...
      info = table[index - 2]
      # text = "#{name}ダイス目（#{dice}）、修正値(#{modify}):最終結果(#{index}):#{info}"
  ...
  def get2d6TableInfo(command)
    ...
    when /BudTT/i
      ...
      table = [
               ...
               '魅力的な裸の異性が、あなたの目の前で誘惑する幻覚を見る。〔性業値〕判定 を行う。「激」になると服を脱ぎだす。もしも外にいればそのエリアの〔治安〕の難易度の〔犯罪〕判定を行う。失敗すると「臭い飯」表を振る。',
