...
・継続判定(成功度+1固定): KCx[±y]
   ...
   y=目標値への修正(任意) x+y-z のように複数指定可能
     ...
     例2）KC6 → 目標値6の継続判定
・罠作動判定: CTR
   罠ごとに1D12を振る。12が出た場合は罠が作動し、獲物がその効果を受ける
・各種表
  ・大失敗表: FT
  ・能力値ランダム決定表: RST
  ・ランダム所要時間表: RTT
...
MESSAGETEXT
  ...
  def getTrapResult()
    ...
    if trapCheckNumber == 12
      ...
      return "罠作動チェック(1D12) ＞ #{trapCheckNumber} ＞ 罠が作動していた！ ＞ 獲物表(#{chaseNumber}) ＞ #{chase}が罠にかかっていた"
    end

    return "罠作動チェック(1D12) ＞ #{trapCheckNumber} ＞ 罠は作動していなかった"
  end
  ...
  @@tables =
    {
      'FT' => {
        ...
        :table => %w{
          ...
          【余裕】が3点減少する（最低0まで）
          【余裕】が3点減少する（最低0まで）
          ランダムな荷物1個が落ちて行方不明になる（大失敗したエリアのアイテム調査で再発見可能）
          ランダムな荷物1個が落ちて行方不明になる（大失敗したエリアのアイテム調査で再発見可能）
          ランダムな荷物1個が破壊される
          ランダムな荷物1個が破壊される
      ...
      'ROMT' => {
        :name => 'ランダム遮蔽物表(屋外)',
        :table => %w{
          【藪】耐久度3,軽減値1,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の補正
          【藪】耐久度3,軽減値1,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の補正
          【木】耐久度5,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の補正
          【木】耐久度5,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の補正
          【木】耐久度5,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の補正
          【大木】耐久度7,軽減値3,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-2の補正
          【大木】耐久度7,軽減値3,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-2の補正
          【大木】耐久度7,軽減値3,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-2の補正
          【岩】耐久度6,軽減値4,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の補正/コンタクト内で行われる格闘攻撃のダメージ+1
          【岩】耐久度6,軽減値4,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の補正/コンタクト内で行われる格闘攻撃のダメージ+1
          【岩壁】耐久度8,軽減値4,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-2の補正/コンタクト内で行われる格闘攻撃のダメージ+2
          【岩壁】耐久度8,軽減値4,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-2の補正/コンタクト内で行われる格闘攻撃のダメージ+2
        }
      },
      ...
      'RIMT' => {
        :name => 'ランダム遮蔽物表(屋内)',
        :table => %w{
          【木材の壁】耐久度4,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の補正
          【木材の壁】耐久度4,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の補正
          【木材の壁】耐久度4,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の補正
          【木材の壁】耐久度4,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の補正
          【木材の扉】耐久度4,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1、接触判定と突撃判定に-2の補正
          【木材の扉】耐久度4,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1、接触判定と突撃判定に-2の補正
          【木材の扉】耐久度4,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1、接触判定と突撃判定に-2の補正
          【木材の扉】耐久度4,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1、接触判定と突撃判定に-2の補正
