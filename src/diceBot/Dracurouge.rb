...
class Dracurouge < DiceBot
  ...
  def prefixs
    ['DR.*', 'RT.*', 'HRT.*', 'CT\d+', "ST", "CO", "CA", "EP", "OS", "PN", "RS"] + @@tables.keys
  end
...
　　　E：将軍、B：僧正
　例）RT（一括表示）、RTDF（ドラクの領主）、RTAN（アヴァロームの野獣）
・異端の反応表（HRTxy）x：血統、y：道　xy省略で一括表示
　血統　L：異端卿、V：ヴルコラク、N：ナハツェーラ、K：カルンシュタイン、
　　　　G：グリマルキン、S：ストリガ、M：メリュジーヌ、F：フォーン
　道　W：野伏、N：流浪、S：密使、H：魔女
　例）HRT（一括表示）、HRTVW（ヴルコラクの野伏）、HRTMH（メリュジーヌの魔女）
・D66ダイスあり
MESSAGETEXT
  end
  ...
  def rollDiceCommand(command)
    ...
    methodList.each do |method|
      result = send(method, command.upcase)
