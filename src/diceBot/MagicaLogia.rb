...
#** 表振り分け
  ...
  def magicalogia_scene_table
    def magicalogia_scene_table_7
      return "周囲で〈断章〉が引き起こした魔法災厄が発生する。#{magicalogia_random_skill_table_text_only}の判定を行うこと。成功すると、好きな魔素が一個発生する。失敗すると「運命変転表」を使用する。"
    end
    ...
    table = [
            ...
            Proc.new(){magicalogia_scene_table_7},
...
#** 変調表
  def magicalogia_wrong_table
    def magicalogia_wrong_table_6
      return "『不運』#{magicalogia_random_skill_table_text_only}のチェック欄をチェックする。その特技が使用不能になり、その分野の特技が指定特技になった判定を行うとき、マイナス1の修正が付く。"
    end
    table = [
            ...
            '『病魔』【防御力】が1点減少する。',
            '『遮蔽』【根源力】が1点減少する',
            Proc.new(){magicalogia_wrong_table_6},
             ]
...
#** 事件表
  def magicalogia_accident_table
    def magicalogia_accident_table_7
      return "生ける屍の群れ、地獄の業火、迷宮化……魔法災厄に襲われる。#{magicalogia_random_skill_table_text_only}の選んで判定を行う。失敗すると、その人物に対し「運命変転表」を使用する。"
    end
    table = [
             ...
             Proc.new(){magicalogia_accident_table_7},
...
#時の流れ表
  def magicalogia_time_passage_table
    ...
    if num == 1
      ...
      tempText, tempNum = magicalogia_random_skill_table
      output = "冒険の日々の途中、大きな幸せが訪れる。#{tempText}の判定を行う。成功すると、自分のアンカーの災厄か、自分の疵一つを無効化する。"
    elsif num == 3
  ...
  def magicalogia_extreme_environment_scene_table
    def magicalogia_extreme_environment_scene_table_7
      return "苛酷な自然環境によってキミたちは窮地に立たされる。#{magicalogia_random_skill_table_text_only}の判定を行うこと。成功すると、なんとか使えそうなものを探し、好きな魔素が一個発生する。失敗すると、キミの負った傷は時間と空間を超えて、キミのアンカーに不幸をもたらす。「運命変転」が発生する。"
    end
    table = [
      ...
      Proc.new(){magicalogia_extreme_environment_scene_table_7},
  ...
  def magicalogia_innner_world_scene_table
    def magicalogia_inner_world_scene_table_7
      return "夜の道。もやもやと黒いものが後ろから、追いかけてくる。はっきりした姿も分からないのに、ひどく恐ろしく感じる。このシーンに登場するPCは、#{magicalogia_random_skill_table_text_only}の判定を行う。成功すると、好きな魔素二個を獲得できる。失敗した場合、もやもやとした黒いものは君のアンカーの前に現れる。「運命変転」が発生する。"
    end
    table = [
      ...
      Proc.new(){magicalogia_inner_world_scene_table_7},
      '夕暮れの街。なんだか寂しい気分になる。',
  ...
  def magicalogia_magic_city_scene_table
    def magicalogia_magic_city_scene_table_7
      return "広場。魔法使い同士が喧嘩を始め、魔法が飛び交い始める。このシーンに登場したPCは、#{magicalogia_random_skill_table_text_only}で判定を行う。成功すると、好きな魔素を二個獲得する。失敗すると、この場所の魔法の影響が時空を超えて、アンカーに現れる。「運命変転」が発生する。"
    end
    table = [
      ...
      Proc.new(){magicalogia_magic_city_scene_table_7},
  ...
  def magicalogia_world_after_dead_scene_table
    def magicalogia_world_after_dead_scene_table_6
      return "床が白い雲になっている場所。雲の隙間からは、現世のような風景が見える。#{magicalogia_random_skill_table_text_only}の判定を行う。成功すると、このセッション中一度だけ、判定のサイコロを振り直すことができる。失敗すると、時空を越えて、アンカーが死後世界を垣間見てしまう。「運命変転」が発生する。"
    end
    table = [
      ...
      Proc.new(){magicalogia_world_after_dead_scene_table_6},
  ...
  def magicalogia_labyrinth_world_scene_table
    def magicalogia_labyrinth_world_scene_table_7
      return "怪物が現れ、キミたちを襲う。このシーンに登場しているPC全員は、#{magicalogia_random_skill_table_text_only}の判定を行うこと。成功すると、怪物を倒し、好きな魔素が一個発生する。失敗すると、キミの負った傷は時間と空間を越えて、キミのアンカーに不幸をもたらす。「運命変転」が発生する。"
    end
    table = [
      ...
      Proc.new(){magicalogia_labyrinth_world_scene_table_7},
