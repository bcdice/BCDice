...
　　一夏のシーン表　IST/セクシーシーン表 EST　
　　ラッキースケベシーン表 RST　/屈服表 XST　
・D66ダイスあり
INFO_MESSAGE_TEXT
  end
  ...
  def sinobigami_scene_table(string)
    ...
    when /JBST/i
     ...
     when /IST/i
      type = '一夏の'
      table = [
               '轟音。そして、一瞬の後、夏の夜空を美しい火花が照らす。',
               '潮騒が聞こえる。そして、懐かしい磯の香り。',
               '蝉の声と虫取り網を持った子どもたち。ヒマワリ香る南風が吹いた。',
               '突然の豪雨。大きな雨粒が降り注ぎ、通りの人々の衣類を濡らす。',
               '見上げると、青い空にそそり立つ入道雲。強い日差しがまぶしい。',
               '熱帯夜。気温は下がったものの蒸れた空気が不快だ。首筋に汗がにじむ。',
               '照りつける太陽。地面からは異様な熱気が立ちのぼり、イヤな渇きを覚えた。',
               '水しぶきとともに嬌声が上がる。きらきらと輝く水面に、あの人の姿を見た。',
               '闇に灯る青い光。誘われた羽虫は、その身を燃やす。',
               '小川から何かが流れてくる。死者の魂を弔う灯籠のようだが…。',
               '浴衣に身を包んだ少女たちが通り過ぎる。どこかで夏祭りでもやっているのだろうか。',
              ]
      when /EST/i
      type = 'セクシー'
      table = [
               '怪しげな香が焚きこめる部屋。頬を上気させ、瞳を潤ませた多くの少女たちが、ゆっくりと近づいてくる。',
               '闇の中で二つの影がゆっくりと蠢く。絹擦れの音と熱い吐息だけが甘く響いていた。',
               'ふぁさり。最後の一枚をベッドの上に脱ぎ捨てる。美しい裸体を見せつけるように体をくねらせると、その人物は艶やかな笑みを浮かべた。',
               '無防備な寝姿。寝返りを打つたびに、衣類がはだけ、滑らかな肌がのぞく。',
               '剣刃一閃。何者かによる目にも止まらぬ一太刀が、衣類だけをはぎ取る。',
               'ちゃぽん。一人の少女が水面にその身を沈めると、波紋が広がり、きらきらとまばゆい輝きを放つ。',
               '激しい雨。濡れた服がぴったりと体に張り付き、下着や身体の線を浮かび上がらせる。',
               '熱いシャワーが降り注ぐ。もうもうと立ち込める湯気の中に、裸身が浮かび上がった。白い肌の上を水滴が転がる。',
               '部屋の中から、すすり泣くようなあえぎが漏れ聞こえた。一体、この扉の向こうで何が行われているのだろう？',
               '屈強な男に足首をつかまれ、少女は抵抗でできずに震えている。少女の怯えた様子を眺めながら、男は下卑た笑いを浮かべた。',
               '無数の屍。濃密な血の匂いが理性を喪失させる。誰かに抱きしめてもらわないと、気が狂いそうだ。',
              ]
when /XST/i
      type = '屈服'
      table = [
               '敗者は、激しく体液をほどばしらせ、糸の切れた操り人形のように突っ伏す。勝者は【生命力】と変調すべて回復する。（「穢れ」を受けていた場合、それを回復することもできる。）',
               '敗者の頭の中で小さな爆発が続けざまに起きた。勝者は、敗者が何かの忍具を持っているか教えてもらい、そのうち一つを奪うことができる。',
               '敗者は、勝者にすがりつき、声を殺してひしとしがみついた。敗者は、そのセッションの間だけ、勝者に対する命中判定と、勝者の攻撃に対する回避判定にマイナス１の修正が付く。',
               '敗者は、体の芯に痺れる様な衝撃が走る。敗者は、ランダムに選んだ変調を一つ受ける。',
               '敗者は、身体を弓なりにそらせ、ぴくぴくと震わせた。勝者は、敗者の持つ【情報】の中から【奥義】以外のものを二つ選び、それを獲得する。',
               '敗者は、身も心も蕩けるような甘美な陶酔のうねりに溺れていた。勝者は、敗者の勝者に対する【感情】を「愛情」か「狂信」にすることができる。',
               '敗者は、かすれた叫びをあげ、四肢をわななかせた。敗者は【生命力】を１点減少する。',
               '敗者は、唇の端から涎を垂らし、壊れた機械の様に痙攣する。勝者は、敗者の持っている【感情】一つを選び、それを失わせることができる。',
               '敗者は、狂おしく身もだえしてさらに勝者を求めた。敗者がNPCだった場合、そのセッションの間だけ、勝者の「従者」にすることができる。',
...
when /RST/i
      type = 'ラッキースケベ'
      table = [
               'なんか熱いんだけど…。偶然口にした飲み物には媚薬が含まれていたみたい。体が火照り、服を脱ぎだす。',
               '忍法の暴走！周囲の人物の服だけがはじけ飛ぶ！',
               'あのコが、勢いよく水からあがってくる。おいおい、水着がとんでもないことになっているぞ！',
               'うう、狭い！気になるあの人と密着することに。その香りとぬくもりに頭がぼぉっとしてくる……。',
               '目の前でかがまれ、襟ぐりから胸元がチラリと見える。ううう、くらくらする。',
               '追っ手から逃げるため、手近な扉へ滑り込む。すると、そこは更衣室だった。着替え中の人物と目が合い、一瞬あと悲鳴があがる。',
               '何かにぶつかり、つまづいてしまう。とっさに伸ばした手が、変なところに……。わ、わざとじゃないんだッ！',
               '一陣の風が吹き、スカートが巻き上がる。慌てておさえて、周りをみわたすと……。',
               '一風呂浴びようと思って扉を開くと、そこには先客が。いや、湯気で肝心なところは見えないけど…。',
               'ひ、ひぃッ！どうしても、これだけはダメ！不意を打たれたあのコは、恐怖のあまり、あなたに抱きついてくる。',
               '全裸の美少女が空から降ってくる。一体、彼女は……ッ！？',
              ]
