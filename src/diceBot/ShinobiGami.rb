...
MESSAGETEXT
  ...
  def dice_command(string, nick_e)
    ...
    return '1', secret_flg unless( /((^|\s)(S)?([CMDTNKG]|TK|GA|KY|JB)?[SFEGWB]T($|\s))/i =~ string )
...
#** テーブル振り分け
  def shinobigami_table(string, nick)
    ...
    when /(FT)/i   # ファンブル表
      return sinobigami_fumble_table(nick);
    when /(ET)/i   # 感情表
...
#** ファンブル表
  def sinobigami_fumble_table(nick)
    table = [
    ...
    return get_sinobigami_1d6_table_output("ファンブル表", table, nick)
  end
