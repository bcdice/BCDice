...
　Windows以外のOSの場合、Ruby1.8をインストールした上で下記のコマンドでライブラリをインストール願います
    gem install net-irc
    gem install wxruby
後はsrcディレクトリに移動し、 ruby -Ku bcdice.rb コマンドで画面が表示されます。後はWindows版と同様です。
...
Ver2.01.03 2011/10/15
・バージョン表記を修正。
・迷宮キングダムのダイスボットに　王国名決定表１～４(KNT1～KNT4）、単語表１～４(NAME1～4)、名前表A(NAMEA),名前表B(NAMEB),エキゾチック名前表(NAMEA),ファンタジック名前表(NAMFA)　を追加。
・迷宮キングダムの判定結果に6が出た場合の気力取得結果を追加。
・サタスペで2d6のような通常のダイスロールが出来ない障害を修正。
