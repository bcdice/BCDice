  ...
  module GameSystem
    class FullFace < Base
      ...
      HELP_MESSAGE = <<~INFO_MESSAGETEXT
        ■判定　x+bFF<=a   x:ヒート(省略時は3) b:判定修正 a:能力値

        例)FF<=2:   能力値2で判定し、その結果(成功数,1の目の数,6の目の数,バースト)を表示。
           6FF<=3:  ヒート6,能力値3で戦闘判定し、その結果( 〃 )を表示。
           8+2FF<=3:ヒート8,判定修正+2,能力値3で戦闘判定し、その結果( 〃 )を表示。

        ■能力値判定　x+bFA<=[a,t]   x:ヒート(省略時は3) b:判定修正 a:能力値 t:難易度(省略可)

        例)FA<=[2]:   能力値2で判定し、その結果(成功数,1の目の数,6の目の数,バースト)を表示。
           FA<=[2,1]: 能力値2,難易度1で判定し、その結果(成功数,1の目の数,6の目の数,成功・失敗,バースト)を表示。

        ■戦闘判定　x+bFB<=[a,t]d   x:ヒート(省略時は3) b:判定修正 a:能力値 t:難易度 d:基本ダメージ(省略可)

        例)6FB<=[3]1:  ヒート6,能力値3,基本ダメージ1で戦闘判定し、その結果(成功数,1の目の数,6の目の数,ダメージ,バースト)を表示。
           8+2FB<=[3]2:ヒート8,判定修正+2,能力値3,基本ダメージ1で戦闘判定し、その結果( 〃 )を表示。

      INFO_MESSAGETEXT

      register_prefix('([+\d]+)*FF', '([+\d]+)*FA', '([+\d]+)*FB')

      def eval_game_system_specific_command(command)
        resolute_action(command) || resolute_ability_action(command) || resolute_battle(command)
      end
      ...
      def resolute_ability_action(command)
        ...
        return Result.new.tap do |result|
          command_out = "(#{heat_level}#{Format.modifier(modify)}FA<=[#{status_no}]"
          if sixs >= 2
            result.fumble = true
            result.condition = false
          else
          ...
          result_txt.push("6の目(#{sixs})") if sixs > 0
          if target_no > 0
            command_out = command_out.chop + ",#{target_no}]"
      ...
      # @param [String] command
      # @return [Result]
      def resolute_battle(command)
        m = /^(\d*)([+\d]+)*FB<=\[(\d)(,(\d))?\](\d)?$/.match(command)
        return nil unless m
        ...
        return Result.new.tap do |result|
          command_out = "(#{heat_level}#{Format.modifier(modify)}FB<=[#{status_no}]"
          if sixs >= 2
            result.fumble = true
            result.condition = false
          else
