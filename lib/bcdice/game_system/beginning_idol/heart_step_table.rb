  ...
  module GameSystem
    class BeginningIdol < Base
      ...
      module WithAbnormality
        REG = /変調がランダムに(一|二|三)つ発生する。/.freeze
        ...
        def replace_abnormality(chosen, randomizer)
          ...
          dice_list = randomizer.roll_barabara(abno_count, 6).sort
          uniq_dice_list = dice_list.uniq

          bad_status_list = uniq_dice_list.map { |index| BadStatusTable.choice(index).body }.join("\n")
          if uniq_dice_list.size > 1
            bad_status_list = "以下の#{uniq_dice_list.size}つが発生する。\n#{bad_status_list}"
          end

          text = "変調 ＞ [#{dice_list.join(',')}] ＞ #{bad_status_list}"
