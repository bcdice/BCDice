  ...
  module GameSystem
    class Revulture < Base
      ...
      def roll_attack(dice_count_expression, border_expression, additional_damage_rules)
        ...
        dices = dice_count > 0 && !(!additional_damage_rules.nil? && border.nil?) ? @randomizer.roll_barabara(dice_count, 6).sort : []
        ...
        if dice_count.zero?
          message_elements << 'ダイス数が 0 です'
        elsif !additional_damage_rules.nil? && border.nil?
          message_elements << '目標値が指定されていないため、追加ダメージを算出できません'
        else
