  ...
  module GameSystem
    class OneWayHeroics < Base
      ...
      def getRandomEventDiceCommandResult(command)
        m = /^RET(\d+)$/.match(command)
        unless m
          return nil
        end

        day = m[1].to_i
        return RANDOM_EVENT_TABLE.roll_with_day(day, @randomizer)
      end

      def getRandomEventPlusDiceCommandResult(command)
        m = /^RETP(\d+)$/.match(command)
        unless m
          return nil
        end

        day = m[1].to_i
        return RANDOM_EVENT_TABLE_PLUS.roll_with_day(day, @randomizer)
      end
      ...
      def getDungeonTableDiceCommandResult(command)
        m = /^DNGN(\d+)$/.match(command)
        unless m
          return nil
        end

        day = m[1].to_i
        return DUNGEON_TABLE.roll_with_day(day, @randomizer)
      end

      def getDungeonPlusTableDiceCommandResult(command)
        m = /^DNGNP(\d+)$/.match(command)
        unless m
          return nil
        end

        day = m[1].to_i
        return DUNGEON_TABLE_PLUS.roll_with_day(day, @randomizer)
      end
      ...
      class MoveToTableWithDay
        ...
        def roll_with_day(day, randomizer)
          <<~RESULT.chomp
            #{@text} ＞
             #{@table_key}#{day} ＞ #{@table.roll_with_day(day, randomizer)}
          RESULT
        end
      end
      ...
      TABLES = {
        ...
        "OUTENC" => DiceTable::ChainTable.new(
          ...
          [
            ...
            "アンデッドの群れ（１５６ページ）。",
            "盗賊の群れ（１５６ページ）。",
            MoveToTable.new("希少動物表（１５６ページ）へ移動。", "RANI"),
          ]
        ),
