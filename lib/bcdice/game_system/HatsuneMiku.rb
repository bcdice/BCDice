      ...
      class << self
        ...
        def translate_tables(locale)
          general_tables(locale)
            .merge(keyword_tables(locale))
            .merge(name_tables(locale))
            .merge(otodama_tables(locale))
        end

        # ファンブル表・致命傷表・休憩表など、通常のテーブル
        # 情景表 (ST) も含む
        ...
        # コア別キーワード表 (DKT/HKT/LKT/EKT/MKT)
        def keyword_tables(locale)
          {
        ...
        # コア別名前表 (DNT/HNT/LNT/ENT/MNT)
        def name_tables(locale)
          {
        ...
        # オトダマ各種表 (OPA/OPB/OHT/OLT)
        def otodama_tables(locale)
          {
