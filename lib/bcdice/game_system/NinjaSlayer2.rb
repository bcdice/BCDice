...
module BCDice
  module GameSystem
    class NinjaSlayer2 < Base
      ...
      HELP_MESSAGE = <<~TEXT
        - K{x1}
        難易度K([K]ids)の成功判定をダイス{x1}個で実行（通常のダイスの{x1}B6>=2と同じ）します。
        先頭の文字を変えることで、難易度E([E]asy),N([N]ormal),H([H]ard),U([U]ltra-hard)もしくはUH([U]ltra-[H]ard)でも実行可能です。

        - K{x1},N{x2},...,U{xn}
        K{x1}の複数ロール版。
        カンマ(,)で区切って複数回入力すると、指定した回数分まとめて判定を行います。
        2回目以降({x2}～{xn})は難易度を省略可能。省略した場合はひとつ前の難易度を引き継いで判定を行います。
        以下のコマンドについても同様の書式で複数ロールしての同時判定が可能です。

        - K{x1}[>={y}]
        K{x1}のロールの結果を使って、[]内で指定された条件による追加判定を行います。
        判定式は「>=」({y}以上)の他に「>」({y}より大きい)、「<=」({y}以下)、「<」({y}未満)、「=」({y}のみ)、「!=」({y}以外)が利用可能です。
        ロール結果の中で条件を満たしたダイスの個数を「追加判定」というテキストと共に出力します。
        [=5][=6]のように複数記述することで、ひとつのロールに対して複数パターンでの追加判定が可能です。

        - K{x1},{x2},...,{xn}[>={y}]
        K{x1}[>={y}]の複数ロール版です。
        {x1}～{xn}の各グループのロール結果に対して、それぞれで判定と結果出力を行います（以下のコマンドも同様のため、以降は省略します）。

        - K{x1}[>={y1}{y2}...{yn}+]
        ロール結果に対し{y1}～{yn}の判定を行い、出目の中に条件を満たしたダイスが**全て**含まれていた場合、「追加判定成功！」というテキストを出力します。
        例えば[>=665+]とした場合、出目の中に6以上のダイスが2つと5以上のダイスが1つ含まれていれば成功扱いになります。

        - K{x1}[S]
        - K{x1}[S{y}]
        K{x1}[>=6{y}]と同様の判定を行ない、条件を満たしたダイスの個数を「サツバツ判定」というテキストと共に出力します。
        {y}を省略した場合は固定値6で処理します。

        - K{x1}[S{y1}{y2}...{yn}+]
        K{x1}[>={y1}{y2}...{yn}+]と同様の判定を行ない、条件を満たしたダイスが**全て**含まれていた場合、「サツバツ！」というテキストを出力します。

        - K{x1}[S{y1}{y2}...{yn}/{z1}{z2}...{zn}+]
        K{x1}[S{y1}{y2}...{yn}+]と同様の処理に加え、{z1}～{zn}でも同様の判定を行ないます。
        {z1}～{zn}で条件を満たした場合、「ナムアミダブツ！」というテキストを出力します。
        {z1}～{zn}の条件を満たせず、{y1}～{yn}で条件を満たした場合、「サツバツ！」というテキストを出力します。

        - K{x1}[C]
        - K{x1}[C{y}]
        K{x1}[>=6{y}]と同様の判定を行ない、条件を満たしたダイスの個数を「クリティカル判定」というテキストと共に出力します。
        {y}を省略した場合は固定値6で処理します。

        - K{x1}[C{y1}{y2}...{yx}+]
        K{x1}[>={y1}{y2}...{yn}+]と同様の判定を行ない、条件を満たしたダイスが**全て**含まれていた場合、「クリティカル！」というテキストを出力します。
