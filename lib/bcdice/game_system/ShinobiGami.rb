      ...
      TABLES = {
        ...
        'HRPT' => DiceTable::Table.new(
          ...
          [
            ...
            '忍災の影響が忍びの世にも及び、この事態を引き起こした人物の責任問題に発展する。今回のセッションで【使命】（【本当の使命】がある場合はそちら）を達成出来なかったPCは「忍務失敗表」（基本p71）を一回使用する。',
        ...
        'OTAT' => DiceTable::Table.new(
          ...
          [
            ...
            '覚醒に伴い、身体の形が変化する。 未覚醒忍法の中からランダムに一つ選び、それを覚醒済忍法にする。',
            '忍者の血に目覚め、力の制御方法が理解出来るようになる！ 好きな未覚醒忍法を一つを選び、それを覚醒済み忍法にする。',
            '自分だけが操ることが出来る独自の力に目覚める。 好きな未覚醒奥義一つを選び、それを覚醒済みの奥義にする。',
            '自分の脳に直接何かが囁きかける！ 集団戦ダメージを１点受ける（退魔編なら代わりに妖魔化する）と、好きな『未覚醒データ』の中からひとつ選び、それを『覚醒済みデータ』にする。'
          ]
        ),
