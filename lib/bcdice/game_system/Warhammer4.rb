  ...
  module GameSystem
    class Warhammer4 < Base
      ...
      HELP_MESSAGE = <<~INFO_MESSAGE_TEXT
        ...
        ■ 『武器を掲げよ！』版クリティカル表 (bkctH, bkctA, bkctB, bkctL, bkctHU, bkctAU, bkctBU, bkctLU)
          "BK部位 頑健ボーナス以下フラグ"の形で指定します。
          部位は「H(頭部)」「A(腕)」「B(胴体)」「L(足)」の４カ所です。
          頑健ボーナスフラグは頑健ボーナス以下のダメージの時にUをつけます。
          例）bkctH bkctAU BKCTL
      ...
      def roll_critical_table(command)
        ...
        case command
          ...
          when /^BKCT([HABTL])(U)?$/
            ...
            under_ganken_bonus = !n[2].nil?
    
            CRITICAL_TABLES_UIA[part].roll(@randomizer, under_ganken_bonus)
        end
      end
      ...
      TABLES = {
        ...
        "WOMMIT" => DiceTable::RangeTable.new(
          ...
          [
            ...
            [11..15, '立ち枯れ：君の【意志力】ボーナス×1 マイル以内の農場が立ち枯れの病にかかり、すべての穀物が一夜のうちに腐れてしまう。'],
            [16..20, '耳垢閉塞：君の耳が即座に分厚い耳垢で詰まってしまう。「耳鳴り状態」1つを得る。誰かが君に耳掃除してくれない限りこれを取り除くことはできない（〈治療〉技能を用いてテストに成功しなければならない）。'],
            [21..25, '凍える吐息：君の周囲の気温が急降下し、人の吐息が白くなる。君から【意志力】ボーナス×1 マイル以内の者は全員手強い (+0) 〈肉体抵抗〉テストに成功しなければ急激な寒さにより、その場を離れない限り【射撃技術度】、【敏捷力】、【器用度】テストに–10の修正を受ける。この効果は1分間継続する。'],
            [26..30, '結び目ほどき：君の「所持品」のあらゆる留め金が外れ、あらゆる紐の結び目がほどけてしまう。ベルトはずり落ち、ポーチは開き、バッグは落ち、鎧も外れてしまうのだ。'],
            [31..35, '気まぐれな装い：君の衣服が自分の意志を持っているかのように振る舞う。「組み付かれた状態」1つを得る。これに抵抗するための【筋力】は1d10×5となる。'],
            [36..40, '節制の呪い：1d100ヤード以内のすべての酒が苦くて臭いひどい味になる。オールド・ワールドの酒場の多くでなぜ呪文詠唱が好まれないのか、その理由の一助である。'],
            [41..45, '鼻につく言葉：1d10ラウンドにわたって、すべての〈言語〉テスト（発動テストも含む）に-10のペナルティを被る。'],
            [46..50, '困惑する：戦闘に参加しているなら、「不意を討たれた状態」になる。それ以外の場合、君は完全にびくついてしまい、心はあたふたとし、ほんの片時も集中することができなくなる。'],
            [51..55, '不浄なる幻視：穢らわしく不浄な行為の幻視が浮かび上がり君を悩ませる。「目がくらんだ状態」1つを得る。「手強い」（+0）〈冷静さ〉テストを行ない、失敗したなら、さらに1つの同「状態」を得る。'],
            [56..60, '魔女の目：君の目が1d10時間にわたって、自分の「魔法体系」に関連した不自然な色に変わる。目の色が変わっている間、君はいかなる手段でも取り除くことができない「目がくらんだ状態」1つを得る。君はこの期間、《魔法感知》と《第二の視覚》異能を使用できなくなる。'],
            [61..65, '破裂：君の鼻、目、耳からおびただしく出血する。1d10個の「出血状態」を得る。'],
            [66..70, '地獄の囁き：GMは印表（p.45）から反転の印を二つ選んでもよい。「普通」（+20）の【意志力】テストを行ない、失敗したなら「堕落ポイント」1点を得る。'],
            [71..75, '恐怖！：君は混沌の領域を忌まわしき瞬く間にいくつも幻視し、それに苛まれる。GMは反転の印を「印表」（p.45）から1つ選んでもよい。「多難」（-20）な〈冷静さ〉テストを行ない、失敗したなら「戦意喪失状態」1つを得る。'],
            [76..80, '堕落の呪い：「堕落ポイント」1点を得る。'],
            [81..85, '腸内大暴動：君はお腹の不調を抑えきれず、粗相してしまう。「疲労状態」1つを得るが、これは君が衣服を取り替え、身体を綺麗にするまで取り除くことができない。'],
            [86..90, '魔法の刻印：君は自身が扱う魔力の風の物理的な印を具現する。ハイシュの印ならp.58、シャモンの印ならp.70、グューランの印ならp.82、アズィルの印ならp.94、ウルグの印ならp.106、シャイシュの印ならp.118、アキュシーの印ならp.130、ガウルの印ならp.142を参照せよ。魔法印表のいずれも君の魔法的な背景に合わないか、すでに具現した印を得るなら、『大誤発動表』でもう一度ロールすること。'],
            [91..95, '倍増する不幸：この表を2回ロールする。91〜100が出た場合には再ロールをすること。'],
            [96..100, 'ほとばしる混沌：『大誤発動表』でもう一度ロールすること。'],
          ]
        ),
        "WOMMAT" => DiceTable::RangeTable.new(
          "大誤発動表（Winds of Magic）",
          "1D100",
          [
            [1..5, '亡霊の声：【意志力】×1ヤード以内のすべての者が“混沌の領域”から現れた暗く誘惑的な囁き声を耳にする。知性のあるクリーチャーは皆、「普通」（+20）の〈冷静さ〉テストを行ない、失敗したなら、「堕落ポイント」1点を得る。'],
            [6..10, 'エーテルの衝撃：君は、【頑健力】ボーナスとアーマー・ポイント〔AP〕による修正値を無視して、1d10点の「耐久値」を失う。さらに、「普通」（+20）の〈肉体抵抗〉テストを行ない、失敗したなら、「朦朧状態」1つも得る。'],
            [11..15, '死の歩み：君の足跡はその行く手に死を残していく。次の1d10時間にわたって、君の周囲のあらゆる植物は枯れ死に果てる。'],
            [16..20, '二重の災難：君が発動した呪文の効果は1d10マイル以内の他の場所で発生する。そこでどのような結果が起こるかはGMの裁定によるが、可能な限り影響があるべきだ。'],
            [21..25, '魂の火：君は自分の「魔法体系」に関連した色の不浄な炎に取り巻かれ、「炎上状態」1つを得る。'],
            [26..30, '抑えの効かぬ舌：君は1d10ラウンドにわたって、わけのわからないことを喋りまくる。この間、君は言葉による意思疏通ができず、あらゆる発動テストを行なうこともできないが、それ以外は通常通り行動してもよい。'],
            [31..35, '群れの襲撃：君はエーテル体のラット、ジャイアント・スパイダー、スネーク、その他（GM の任意）の群れに囲まれる。関連するクリーチャー種別の通常プロフィールに「群れ」の「クリーチャー特徴」を加えて用いること。1d10ラウンドの終了後に、まだ倒されていなければその群れは撤退する。'],
            [36..40, '布人形：君はランダムな方向に1d10ヤード空中に投げ飛ばされ、地面に落ちて1d10点の「耐久値」を失い（アーマー・ポイント〔AP〕は無視する）、さらに「伏せ状態」になる。'],
            [41..45, '凍りつく四肢：四肢のひとつ（ランダムに決定する）が1d10ラウンドにわたって動かなくなる。この四肢はまるで「身体欠損」（P.180を参照）になったかのように使い物にならない。'],
            [46..50, '闇の目：君は1d10時間にわたって、《第二の視覚》の異能の利益を失う。この時間中、君は〈魔風交信〉テストにも-20のペナルティを被る。'],
            [51..55, '混沌の先見：1d10点ぶんの「幸運ポイント」の余剰プールを得る（これは通常の上限を超えても構わない）。君が「幸運ポイント」を1点消費するたびに、「堕落ポイント」を1点獲得する。なお、セッションの終了時に残存している追加で得たポイントはすべて失われる。GMは君の【意志力】ボーナスと等しい数の反転の印（p.45）を君に与える。'],
            [56..60, '浮遊：君は“魔力の風”に乗り、1d10分にわたって地上から1d10ヤードの高さに浮かび上がる。他のキャラクターたちが君を無理やり動かすこともできるし、君も呪文や翼などを使って移動することもできるが、そのままにしておくと君が浮かんだ位置に常に戻ってくる。「浮遊」が終了した後に何が起こるかについては、「落下」（P.166）を参照のこと。'],
            [61..65, '吐き気：君は吐き気が抑えきれなくなり、君の体内にあった量以上の、悪臭を放つ汚物を吐きちらす。「朦朧状態」1つを得る。これは1d10ラウンドにわたって持続する。'],
            [66..70, '混沌の地震：1d100ヤード以内にいるすべてのクリーチャーは「普通」（+20）の〈運動〉テストを行ない、失敗したなら、「伏せ状態」になる。'],
            [71..75, '忘却：発動しようとした呪文が君の記憶から喪失する。君がもしその呪文を記憶していたなら、思い出すことができず、再度記憶せねばならない。魔導書から発動していたなら、呪文が書かれていたページが発火する。呪文は失われ、適切な処置をすぐに行わなければ、魔導書が「炎上状態」1つを得る。'],
            [76..80, '裏切り：暗黒の神々が恐るべき背信を行なうよう君を誘惑する。君が味方の1人に対する攻撃か、その他何らかの裏切り行為に全力を尽くしたなら、「幸運ポイント」をすべて回復できる。君が他のキャラクターに「運命点」1点を失わせたなら、君がその1点の「運命点」を得る。'],
            [81..85, '穢らわしい弱体化：1点の「堕落ポイント」に加えて、「疲労状態」1つを獲得し、さらに「伏せ状態」にもなる。'],
            [86..90, '凄まじき悪臭：いまや君は鼻が曲がりそうなまでに臭い！君は「攪乱」の「クリーチャー特徴」（P.338）を獲得し、おそらく嗅覚を持つ誰彼すべてからの敵意にさらされる。この効果は1d10時間持続する。'],
            [91..95, '力の流出：君は1d10分にわたって、呪文を発動するために使う異能を使用することができない（通常は《秘術魔術》だが、《混沌魔術》や同種の異能ということもありえる）。'],
