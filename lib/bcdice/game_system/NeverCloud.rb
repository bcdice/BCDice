    ...
    class NeverCloud < Base
      ...
      TEXTS = {
        'LIST' => <<~TEXT,
          ...
          ・判定(xD6±y>=z)
          2D6(CHAR1)　・所属表(2D6またはRoC。基本ルールブック書籍版P33)
          2D6(CHRA2)　・趣味表(2D6またはRoC。基本ルールブック書籍版P38)
          2D6(CHAR3)　・リビド武装形状表(2D6またはRoC。基本ルールブック書籍版P39)
          2D6(NAYA1)　●悩みの詳細表・愛/Love:得意方向(正面)(2D6またはRoC。基本ルールブック書籍版P49)
          2D6(NAYA2)　●悩みの詳細表・体/Figure:得意方向(正面)(2D6またはRoC。基本ルールブック書籍版P49)
          2D6(NAYA3)　●悩みの詳細表・才/Talent:得意方向(背面)(2D6またはRoC。基本ルールブック書籍版P49)
          2D6(NAYA4)　●悩みの詳細表・絆/Bonds:得意方向(側面)(2D6またはRoC。基本ルールブック書籍版P49)
          2D6(NAYA5)　●悩みの詳細表・住/Home:得意方向(側面)(2D6またはRoC。基本ルールブック書籍版P49)

          (LIKE)　◆LIKE(基本ルールブック書籍版P60)
          (RESE1)　◆リサーチカード(基本ルールブック書籍版P63)
          (RESE2)　◆インタビュー(基本ルールブック書籍版P62)
          (RESE3)　◆パイルドライヴ(基本ルールブック書籍版P64)
          (RESE4)　●[決意]と[使命](基本ルールブック書籍版P61)

          (ARTS)　◆リビドアーツのデータ項目(基本ルールブック書籍版P40)
          (STAT1)　●SSリスト(基本ルールブック書籍版P41)
          (STAT2)　●LSリスト(基本ルールブック書籍版P411)
          (ACTI1)　●タイミング:サブアクション(基本ルールブック書籍版P422)
          (ACTI2)　●タイミング:メインアクション(基本ルールブック書籍版P42)
          (ACTI3)　●タイミング:インスタント(基本ルールブック書籍版P42)
          (ACTI4)　●サーヴァント共通アクション(基本ルールブック書籍版P42)

          (BATT1)　◆ラストパイル(基本ルールブック書籍版P67)
          (BATT2)　■攻撃の手順(メインアクションで行う)●01.攻撃宣言ステップ(基本ルールブック書籍版P74)
          (BATT3)　■攻撃の手順(メインアクションで行う)●02.命中判定ステップ(基本ルールブック書籍版P74)
          (BATT4)　■攻撃の手順(メインアクションで行う)●03.ダメージ決定ステップ(基本ルールブック書籍版P74)
          (BATT5)　■攻撃の手順(メインアクションで行う)●04.ダメージ適用ステップ(基本ルールブック書籍版P74)
          (BATT6)　●[決意]の効果(基本ルールブック書籍版P61)
          (BATT7)　●[使命]の効果(基本ルールブック書籍版P61)
          (BATT8)　■パケット(基本ルールブック書籍版P69)
          (BATT9)　■リビドストーム(基本ルールブック書籍版P71)
          (PIET1)　◆ピーターの基本的な性質(基本ルールブック書籍版P72)
          (PIET2)　《AoE》(Area_of_Effect)(基本ルールブック書籍版P72)
          (PIET3)　◆ピーターの「行動指針」((2D6またはRoC。基本ルールブック書籍版P110)
        ...
        'RESE3' => <<~TEXT,
          ...
          「難易度」以上ならば成功(パイルドライヴ)として、PCは[30]のHPダメージを受け「ウラ面」を開示し[Pコトノハ]を手に入れる＿
          ・指定されたサイクル以内に「ウラ面」を開示できなかった
          　リサーチカードの「ピーターのバフ」は戦闘シーンの〈ピーター〉に付与される。
        TEXT
        'RESE4' => <<~TEXT,
          ●[決意]と[使命](基本ルールブック書籍版P61)
        ...
        TEXT
        'BATT4' => <<~TEXT,
          ■攻撃の手順(メインアクションで行う)●03.ダメージ決定ステップ(基本ルールブック書籍版P74)
        ...
        TEXT
        'BATT5' => <<~TEXT,
          ■攻撃の手順(メインアクションで行う)●04.ダメージ適用ステップ(基本ルールブック書籍版P74)
        ...
        TEXT
        'MINI1' => <<~TEXT,
          ■基本ルールからねばくらミニへの変更点(ねばくらミニ書籍版P05)＿
