    ...
    class SajinsenkiAGuS2E < SajinsenkiAGuS
      ...
      SECOND_ED_TABLES = {
        ...
        "SPE" => DiceTable::Table.new(
          ...
          "1D10",
          [
            '0:雨 雨は砂を土へと変えてしまう。R中、全ユニット移動/突撃不可。',
            '1:混戦 以下のエリアのユニットをシャッフルする。♠：A⇔C ♣：B⇔D ♦：A⇔D ♥：B⇔C',
            '2:乱戦 R中、すべての攻撃は［距離：○］になる。',
            '3:逸失 敵拠点エリアのユニットを［♠♣：A ♦♥：B］に移動。味方拠点エリアのユニットを［♠♣：D ♦♥：C］に移動。',
            '4:突風 艦船、オブジェクト以外の全ユニットを「風向き」方向に移動。',
            '5:流砂 以下のエリアのユニットは脱出のため、MPとEPを［3］点失う。［♠：A ♣：B ♦：C ♥：D］',
            '6:混乱 母船内でトラブル発生。R中、【整備】は行えない。［♠♥：味方側 ♣♦：敵側］',
            '7:岩盤 以下のエリアのユニットは岩盤に乗り上げ、《クリティカル》が1回発生。［♠：A ♣：B ♦：C ♥：D］',
            '8:混乱 R中、イニシア値を逆順で処理する。 ※【エイミング】等の高低も逆として処理する。',
            '9:飛礫 飛礫によって、すべてのユニットはAPを［1D10］（以下のエリアでは［2D10］）点失う。［♠：A ♣：B ♦：C ♥：D］',
          ]
        ),
      }.freeze
