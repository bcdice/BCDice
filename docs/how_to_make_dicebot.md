...
テストデータの作成にTOMLの詳細を知る必要はありませんが、気になったり、記述に問題が発生したら仕様を確認してみてください。
...
`input` はテストしたコマンドを書き、それに対応する出力を `output` に記述します。
...
テストデータを書くべきポイントは「おおよその出力パターンを網羅できているか」と「特殊なケースを網羅できているか」の２点です。
`nOT>=x` を例に考えてみましょう。
...
まずはコマンドの登録をする必要があります。 `register_prefix` を以下のように修正しましょう。
...
この文字列は正規表現相当の文字列を登録します。正規表現で `\d+` は数字を1文字以上を表しています。正規表現の詳しい記述はインターネットで検索してみてください。
...
ダイスロールして表を参照する、ダイス表は頻出のためBCDiceで簡単に実装できるツールを用意しています。
